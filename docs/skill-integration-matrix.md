...
| `plan-lab-experiment` 0.2.1 | Experiment Planner | one supported hypothesis and policy | one-variable plan, budget, rollback | unsafe scope → `REJECTED/BLOCKED` | add allowlisted bounded change pattern |
...
完整七 Skill 契约、Agent/Skill/Tool 分层、I/O 校验和未来 live usage event 边界见
`docs/skill-framework.md`。历史 AT-004 不包含 `skill_id` event，不得从角色名反推调用证据。
