...
**画面：** 标题页；随后快速切到 Agent 可修改配置、调用工具的抽象图。

**旁白：**

> 当 Agent 从“回答问题”变成“调用工具并执行工程动作”，关键问题不再只是它聪不聪明，而是：
> 它是谁、能做什么、谁批准、真正执行了什么，以及结果是否由独立角色验证。
...
> Manager 请求独立人工审批；Safe Executor 才能调用 Gateway。Runner 在 CPU、禁网、只读根文件
